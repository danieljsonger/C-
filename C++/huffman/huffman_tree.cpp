                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(singleQueue.front() != NULL && mergeQueue.front() != NULL){
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    TreeNode * currSmall_1;
    TreeNode * currSmall_2;

    while(singleQueue.size() + mergeQueue.size() > 1){
      currSmall_1 = removeSmallest(singleQueue,mergeQueue);
      currSmall_2 = removeSmallest(singleQueue,mergeQueue);
      TreeNode * internal = new TreeNode(currSmall_1->freq.getFrequency()+currSmall_2->freq.getFrequency());
      internal->left = currSmall_1;
      internal->right = currSmall_2;
      mergeQueue.push(internal);
    }

    this->root_ = removeSmallest(singleQueue,mergeQueue);
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
      ...
      if (current->left == NULL && current->right == NULL) {
        ss << current->freq.getCharacter(); // leaf node
        current = root_;
      }
        /**
         * @todo Your code here!
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.hasBits()){
        ...
        if(curr_bit){
            ...
            TreeNode * temp = new TreeNode(new_freq);
            return temp;
        } else if(!curr_bit) {
