...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    // THIS WORKS
    // unsigned x = 0;
    // while((x < elements.size()) && (elements[x] < val)) { x++; }

    // Binary Search Algorithm
    // THIS DOES NOT WORK
    unsigned max = elements.size() - 1;
    unsigned min = 0;
    unsigned x = ((max - min) >> 1) + min;

    while (!(elements[x] == val)) {
        if (elements[x] > val) { max = x - 1; }
        else if (elements[x] < val) { min = x + 1; }
        x = ((max - min) >> 1) + min;
        std::cout << "x = " << x << "\nmax = " << max << "\nmin = " << min << std::endl;
    }
