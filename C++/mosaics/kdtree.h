...
class KDTree
{
  ...
  private:
     ...
     */
    KDTreeNode* _copy(KDTreeNode * other);
    void _destructor(KDTreeNode * subRoot);
    int distance(const Point<Dim> &first, const Point<Dim> &second) const;
    void swap(int index1, int index2, vector<Point<Dim>> &newPoints);
    void partition(int left, int right, int pivot, vector<Point<Dim>> &newPoints, int dim);
    void traverse(int left, int right, vector<Point<Dim>> &newPoints, int dim, KDTreeNode *&node);
    void traverseNearest(const Point<Dim> &query, Point<Dim> &best, int dim, KDTreeNode *current) const;
};
